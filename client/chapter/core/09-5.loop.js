...
// iterable(반복 가능한) = Sting, Array, Array-like(유사배열) = iterable한 요소들 
// 객체는 iterable 하지 않기 떄문에 for of 를 사용할 수 없다.
// 객체에 iterable을 부여하기 위해 symbol을 사용한다. 이러면 for of를 사용할 수 있다.
...
for(let value of languages){
  ...
  if(name.includes('Java') && name.length < 5) break; // JavaScript
...
for(let key in randomUser){
   ...
   // console.log(randomUser[key]); : 조상까지 가져오므로 이렇게 조회 금지
