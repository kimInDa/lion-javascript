...
const shopOrder = {
  ...
  totalPrice(){
    ...
    return this.menu.reduce((acc,item)=> acc + (item.price * item.count) ,0) // 화살표 함수가 암묵적으로 return한 값을 제일 앞쪽 return이 받아서 씀.
  }
};

// forEach
// reduce
// ✨reduce와 map은 return이 필수!
...
// 일반 함수 : 나를 호출한 대상(.)을 this를 바인딩.
// 화살표 함수 : this를 바인딩하지 않음. 상위 컨텍스트에서 this를 가져옴.