...
function createUserCard({
  id = '',           // 아무값도 받지 뭇했을 때, 기본값을 설정하여 undefined 방지
  name = 'tiger', 
...
function createSpinner(size = 100, loadingMessage = '유저 정보를 가져오는 중...📂'){
  return /* html */ `
    ...
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin:auto;background:#fff;display:block;" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" preserveAspectRatio="xMidYMid">
      <defs>
        <filter id="ldio-unlanbi1drs-filter" x="-100%" y="-100%" width="300%" height="300%" color-interpolation-filters="sRGB">
      ...
      <g filter="url(#ldio-unlanbi1drs-filter)"><g transform="translate(50 50)">
      <g>
...
export function rendderEmptyCard(target){
